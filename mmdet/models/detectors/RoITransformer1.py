...
class RoITransformer1(BaseDetectorNew, RPNTestMixin):
    ...
    def __init__(self,
        ...
        super(RoITransformer1, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(RoITransformer1, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        if self.with_rpn:
            ...
            p2 = rpn_outs[0]
            scale_k_2 = self.scale_conv_k_2(p2)
            scale_q_2 = scale_k_2
            scale_v_2 = self.scale_conv_v_2(p2)
            m_batchsize, C, height, width = scale_k_2.size()
            scale_q_2 = scale_q_2.view(m_batchsize, C, -1)
            scale_k_2 = scale_k_2.view(m_batchsize, C, -1).permute(0, 2, 1)
            energy = torch.bmm(scale_q_2, scale_k_2)
            energy_new = torch.max(energy, -1, keepdim=True)[0].expand_as(energy) - energy
            attention = F.softmax(energy_new)
            proj_value = scale_v_2.view(m_batchsize, C, -1)
